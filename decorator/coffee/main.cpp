...
int main()
{
	...
	// Подробнее рассмотрим работу MakeCondiment и оператора <<
	{
		...
		auto tea = make_unique<CGreenTea>();
		...
		*/
		...
		auto oneMoreLemonIceTea =
			make_unique<CEarlGreyTea>()	// Берем чай
	...
	{
		auto beverage = 
			make_unique<CSmallPortionMilkshake>()					// Наливаем молочный коктейль
	...
	{
		auto beverage =
			make_unique<CDoubleLatte>()							// Наливаем чашечку латте,
			<< MakeCondiment<CCream>()					// оборачиваем корицей,
			<< MakeCondiment<CSlicesChocolate>(4)						// добавляем пару долек лимона
			<< MakeCondiment<CLiquor>(LiquorType::Hazelnut)// брасаем пару кубиков сухого льда
			<< MakeCondiment<CChocolateCrumbs>(2);			// посыпаем шоколадной крошкой

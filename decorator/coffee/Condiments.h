...
// Дольки шоколада
class CSlicesChocolate : public CCondimentDecorator
{
public:
	CSlicesChocolate(IBeveragePtr && beverage, unsigned numLobes)
		:CCondimentDecorator(move(beverage))
	{
		m_numLobes = numLobes;
		if (numLobes > 5)
	...
	double GetCondimentCost()const override
	{
		return 10 * m_numLobes;
	}

	std::string GetCondimentDescription()const override
	{
		return "Slices Chocolate - " + std::to_string(m_numLobes) + " lobes";
	}
private:
	unsigned m_numLobes;
};
...
protected:
	...
	std::string GetCondimentDescription()const override
	{
		return std::string(m_liquorType == LiquorType::Chocolate ? "Chocolate" : "Hazelnut")
			+ " syrup";
	}
private:
